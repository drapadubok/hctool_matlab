...
cfg.lambda = 0.2154;
njobs = 50; % how many jobs in array
shuffle_group = 0;
...
for t = 1:length(tokennames)
    ...
    for s = 1:length(receivers)
        ...
        if ~exist(sprintf('%s/hyper/%s/%s/%s/perms_cca',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver),'dir');
            mkdir(sprintf('%s/hyper/%s/%s/%s/perms_cca',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver));
        end
        save(sprintf('%s/hyper/%s/%s/%s/permcfg_cca',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver),'cfg');       
        
        % job filename
        filename = sprintf('%s/hyper/%s/%s/%s/permjob_cca',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver);            
        delete(filename);
        % log filename
        logfile = sprintf('%s/hyper/%s/%s/%s/permlogfile_cca',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver);
        %load the modules
        dlmwrite(filename, '#!/bin/sh', '');
        % Handle the long jobs
        dlmwrite(filename, '#SBATCH -p batch','-append','delimiter','');
        dlmwrite(filename, '#SBATCH -t 8:00:00','-append','delimiter','');
        ...
        %command            
        dlmwrite(filename,sprintf('Rscript bccaPerms.r "%s/hyper/%s/%s/%s/permcfg_cca.mat" %i $SLURM_ARRAY_TASK_ID',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver,shuffle_group),'-append','delimiter','');
