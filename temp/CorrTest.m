...
%         % Take transformation
%         cca = load(sprintf('%s/hyper/%s/%s/%s/ccafit_K50_cvi%i_4mm.mat',dataroot,maskname,actor,receivers{s},r));
...
for s = 1:length(receivers)
    ...
    load(sprintf('%s/%s/%s_corrtest.mat',dataroot,receivers{s},maskname));
    for v = 1:length(fidx)
        [rmat(v,s),pmat(v,s)] = corr(storedA{1}(:,v),storedO(:,v));
        [proj_rmat(v,s),proj_pmat(v,s)] = corr(storedA{1}(:,v),storedProj(:,v));  
    end
end
