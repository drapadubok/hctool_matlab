...
for t = 1:length(tokennames)
    ...
    for s = 1:length(receivers)
        ...
        save(sprintf('%s/hyper/%s/%s/%s/permcfgbin',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver),'cfg');
        
        % job filename
        filename = sprintf('%s/hyper/%s/%s/%s/permjobbin',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver);            
        delete(filename);
        % log filename
        logfile = sprintf('%s/hyper/%s/%s/%s/permlogfilebin',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver);
        %load the modules
        ...
        %command
        dlmwrite(filename,sprintf('matlab -nosplash -nodisplay -nojvm -nodesktop -r "mvlaPermutation(''%s/hyper/%s/%s/%s/permcfgbin'',%i,''hyper'',$SLURM_ARRAY_TASK_ID);"',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver,shuffle_group),'-append','delimiter','');
