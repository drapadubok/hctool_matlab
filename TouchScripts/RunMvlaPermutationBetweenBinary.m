...
for t = 1:length(tokennames)
    ...
    for s = 1:length(receivers)
        ...
        save(sprintf('%s/between/%s/%s/%s/permcfgbin',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver),'cfg');
        % job filename
        filename = sprintf('%s/between/%s/%s/%s/permjobbin',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver);
        % log filename
        logfile = sprintf('%s/between/%s/%s/%s/permlogfilebin',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver);
        %load the modules
        ...
        %command
        dlmwrite(filename,sprintf('matlab -nosplash -nodisplay -nojvm -nodesktop -r "mvlaPermutation(''%s/between/%s/%s/%s/permcfgbin'',%i,''between'',$SLURM_ARRAY_TASK_ID);"',cfg.dataroot,cfg.token,cfg.sender,cfg.receiver,shuffle_group),'-append','delimiter','');
